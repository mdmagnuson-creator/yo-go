 ...
 */
export async function scanCodebase(projectRoot, config, filterFiles) {
 ...
 */
export async function chunkFiles(files, strategy) {
    const chunks = [];
 ...
 */
function canParseWithAST(language) {
    if (!Parser)
        return false;
 ...
 */
async function chunkWithAST(file) {
    const chunks = [];
 ...
 */
function extractSemanticNodes(rootNode, language) {
    const nodes = [];
 ...
 */
function getSemanticNodeTypes(language) {
    switch (language) {
        case 'typescript':
        case 'javascript':
            return [
 ...
 */
function chunkWithSlidingWindow(file) {
    const chunks = [];
 ...
 */
function splitLargeChunk(content, file, startLine) {
    const chunks = [];
