 ...
 */
export async function hybridSearch(indexDir, query, options = {}) {
    ...
    const { embedding: queryVector } = await generateQueryEmbeddingAuto(query, options.embedding || {});
    // Vector search
 ...
 */
export async function vectorSearch(indexDir, query, options = {}) {
    const topK = options.topK || 20;
    const { embedding: queryVector } = await generateQueryEmbeddingAuto(query, options.embedding || {});
    const store = new VectorStore(indexDir);
    await store.initialize();
 ...
 */
export async function keywordSearch(indexDir, query, options = {}) {
    const topK = options.topK || 20;
 ...
 */
export async function hybridSearchRRF(indexDir, query, options = {}) {
    const topK = options.topK || 20;
