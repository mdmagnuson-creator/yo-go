 ...
 */
export async function getChangedFilesSinceIndex(projectRoot, lastIndexedCommit) {
    ...
    try {
        ...
        const { stdout } = await execAsync(`git diff --name-only ${lastIndexedCommit}...HEAD`, { cwd: projectRoot });
 ...
 */
export async function hasUncommittedChanges(projectRoot) {
    try {
 ...
 */
export async function installGitHook(projectRoot) {
 ...
 * Uninstall git hooks
 */
export async function uninstallGitHook(projectRoot) {
    const hooksDir = path.join(projectRoot, '.git', 'hooks');
 ...
 */
async function writeHookFile(hookPath, script) {
    if (fs.existsSync(hookPath)) {
 ...
 */
export async function getLastCommitFiles(projectRoot) {
    try {
        const { stdout } = await execAsync('git diff-tree --no-commit-id --name-only -r HEAD', { cwd: projectRoot });
 ...
 */
export async function getGitIgnorePatterns(projectRoot) {
