 ...
 */
export function matchesPattern(tableKey, include, exclude) {
    // Check exclude first
    for (const pattern of exclude) {
        if (matchGlob(tableKey, pattern))
            return false;
    }
    // Then check include
 ...
 */
export function groupByTable(rows, tableKey = 'table_name', schemaKey = 'table_schema') {
    const tables = {};
 ...
 */
export function formatTableDDL(tableName, columns, foreignKeys, indexes, rlsPolicies) {
    const lines = [];
 ...
 */
export function formatConfigRows(tableName, rows, skipBinaryColumns = true) {
    if (rows.length === 0)
    ...
    const columns = skipBinaryColumns
        ? allColumns.filter(col => {
            ...
            if (Buffer.isBuffer(sampleValue))
                return false;
 ...
 */
export function createSchemaChunks(tables, foreignKeys, indexes, config, rlsPolicies) {
    const chunks = [];
    ...
    for (const [tableKey, columns] of Object.entries(tables)) {
        ...
        if (!matchesPattern(tableKey, include, exclude))
            continue;
 ...
 */
export async function extractDatabaseSchema(connectionUrl, config) {
 ...
 */
export async function extractConfigTableRows(connectionUrl, config) {
    if (!config?.configTables?.length)
        return [];
    const dbType = detectDatabaseType(connectionUrl);
    const chunks = [];
...
// ============================================================================
async function extractPostgresSchema(connectionUrl, config) {
...
async function extractMysqlSchema(connectionUrl, config) {
    ...
    `, [dbName]);
        const foreignKeys = groupForeignKeys(fkRows);
...
async function extractSqliteSchema(connectionUrl, config) {
    ...
    `).all();
        ...
        for (const { name } of tables) {
            ...
            tableSchemas[name] = columns.map(col => ({
                table_schema: 'main',
                table_name: name,
...
async function fetchConfigRows(connectionUrl, dbType, tableConfig) {
