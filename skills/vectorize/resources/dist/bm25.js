 ...
 */
export async function buildBM25Index(indexDir, chunks) {
    ...
    // Save index
    fs.writeFileSync(path.join(bm25Dir, 'index.json'), JSON.stringify(index, null, 2));
    ...
    fs.writeFileSync(path.join(bm25Dir, 'term-chunks.json'), JSON.stringify(termChunks));
}
/**
 * Search BM25 index
 */
export async function searchBM25(indexDir, query, topK = 20) {
    ...
    for (const docId of candidateDocs) {
        ...
        for (const term of queryTokens) {
            if (!index.termFreqs[term] || !index.termFreqs[term][docId])
                continue;
            ...
            const tfNorm = (tf * (index.k1 + 1)) / (tf + index.k1 * (1 - index.b + index.b * docLength / index.avgDocLength));
            score += idf * tfNorm;
        }
        if (score > 0) {
 ...
 */
export function getBM25Stats(indexDir) {
    const indexPath = path.join(indexDir, 'bm25', 'index.json');
    if (!fs.existsSync(indexPath)) {
        return null;
    }
