 ...
 */
export async function extractRelationships(files) {
    const relationships = [];
    // First pass: collect all function definitions
    const functionDefs = new Map();
    const importMaps = new Map(); // file -> (localName -> sourcePath)
    for (const file of files) {
        if (!canAnalyze(file.language))
            continue;
        try {
 ...
 */
function extractFunctionDefinitions(rootNode, file) {
    const defs = [];
    function traverse(node, parentClass) {
 ...
 */
function extractImports(rootNode, file) {
    const imports = [];
                            ...
                            if (child.type === 'import_clause') {
                                for (const spec of child.children || []) {
                                    if (spec.type === 'identifier') {
                                        ...
                                        const alias = spec.children?.find((c) => c.type === 'identifier')?.text;
                                        if (alias)
 ...
 */
function extractFunctionCalls(rootNode, file, importMap) {
    const calls = [];
 ...
 */
function extractCalleeExpression(node, language) {
    switch (language) {
        case 'typescript':
        case 'javascript': {
 ...
 */
function extractClassRelationships(rootNode, file) {
    const relationships = [];
    function traverse(node) {
        switch (file.language) {
            case 'typescript':
                        ...
                        if (child.type === 'class_heritage') {
                            for (const heritageChild of child.children || []) {
                                if (heritageChild.type === 'extends_clause') {
                                    const extended = heritageChild.children?.find((c) => c.type === 'identifier')?.text;
 ...
 */
export function findCallers(relationships, functionName, file) {
 ...
 */
export function findCallees(relationships, functionName, file) {
    return relationships.filter(r => r.relationshipType === 'calls' &&
 ...
 */
export function findImporters(relationships, modulePath) {
    return relationships.filter(r => r.relationshipType === 'imports' &&
