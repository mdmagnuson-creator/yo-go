 ...
 */
export function detectTestType(filePath, content) {
 ...
 */
export function extractTests(file) {
 ...
 */
function extractJsTests(file, testType) {
    const tests = [];
    ...
    try {
        ...
        function traverse(node, describePath = []) {
            ...
            if (node.type === 'call_expression') {
                ...
                if (callee) {
                    ...
                    if (calleeName === 'describe' || calleeName === 'context') {
                        ...
                        const nameArg = describeArgs.find((c) => c.type === 'string' || c.type === 'template_string');
                        const describeName = nameArg?.text?.replace(/^['"`]|['"`]$/g, '') || '';
                        // Find the callback body and traverse it
                        const callbackArg = describeArgs.find((c) => c.type === 'arrow_function' || c.type === 'function_expression');
                        if (callbackArg) {
                    ...
                    if (calleeName === 'it' || calleeName === 'test' ||
                        ...
                        const nameArg = testArgs.find((c) => c.type === 'string' || c.type === 'template_string');
 ...
 */
function extractJsTestsRegex(file, testType) {
    const tests = [];
 ...
 */
function extractPythonTests(file, testType) {
    const tests = [];
    if (!Parser || !Python) {
 ...
 */
function extractPythonTestsRegex(file, testType) {
    const tests = [];
    const lines = file.content.split('\n');
 ...
 */
function extractGoTests(file, testType) {
    const tests = [];
    if (!Parser || !Go) {
 ...
 */
function extractGoTestsRegex(file, testType) {
    const tests = [];
    const lines = file.content.split('\n');
 ...
 */
export function extractTestImports(file) {
    const imports = [];
    if (!Parser) {
 ...
 */
function extractTestImportsRegex(file) {
    const imports = [];
    const lines = file.content.split('\n');
 ...
 */
export async function createTestMappings(testFiles, projectRoot) {
    const mappings = [];
 ...
 */
export function findTestsForFile(mappings, sourceFile) {
    return mappings.filter(m => m.sourceFile === sourceFile ||
 ...
 */
export function findTestsForFunction(mappings, sourceFile, functionName) {
    return mappings.filter(m => (m.sourceFile === sourceFile || m.sourceFile.includes(sourceFile)) &&
        (m.sourceName === functionName || !m.sourceName));
}
/**
 * Parse coverage report (lcov format) to enhance mappings
 */
export function parseLcovCoverage(lcovPath) {
 ...
 */
export function extractE2ERoutes(file) {
    const routes = [];
    const lines = file.content.split('\n');
