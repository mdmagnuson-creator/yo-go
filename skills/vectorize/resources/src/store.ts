...
export class VectorStore {
   ...
   */
  async addRelationships(records: RelationshipRecord[]): Promise<void> {
    if (!this.db) throw new Error('Store not initialized');
   ...
   */
  async addGitHistory(records: GitHistoryRecord[]): Promise<void> {
    if (!this.db) throw new Error('Store not initialized');
    if (records.length === 0) return;
   ...
   */
  async addTestMappings(records: TestMappingRecord[]): Promise<void> {
    if (!this.db) throw new Error('Store not initialized');
    if (records.length === 0) return;
  ...
  async queryTestMappings(
    ...
    try {
      ...
      const results = await query.toArray();
      return results as TestMappingRecord[];
    } catch {
      return [];
    }
  }
  ...
  async getStats(): Promise<{
    ...
    const tableCounts: [string, keyof typeof stats][] = [
