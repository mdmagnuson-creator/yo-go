 ...
 */
function extractExports(file: FileInfo): string[] {
 ...
 */
function extractImports(file: FileInfo): string[] {
 ...
 */
export function estimateSummaryCost(modules: ModuleInfo[]): { inputTokens: number; outputTokens: number; estimatedCost: number } {
  // Rough estimates:
 ...
 */
export function buildModuleSummaryPrompt(module: ModuleInfo, files: FileInfo[]): string {
  // Get relevant files
 ...
 */
export function buildProjectSummaryPrompt(modules: ModuleInfo[], projectRoot: string): string {
...
`;
  ...
  const moduleOverview = modules.slice(0, 10).map(m => 
    `- ${m.name} (${m.path}): ${m.files.length} files, exports: ${m.exports.slice(0, 5).join(', ')}`
  ).join('\n');
...
export async function generateSummaries(
  ...
  modules: ModuleInfo[],
  files: FileInfo[],
  options: Partial<SummaryGeneratorOptions> = {}
...
function generateBasicSummary(
  ...
  if (hasNextjs) projectType = 'Next.js application';
  else if (hasReact && hasExpress) projectType = 'Full-stack React + Express application';
  else if (hasReact) projectType = 'React application';
  else if (hasExpress) projectType = 'Express.js backend';
 ...
 */
export function loadSummaries(indexDir: string): ArchitectureSummary | null {
 ...
 */
export function saveSummaries(indexDir: string, summaries: ArchitectureSummary): void {
  const summaryPath = path.join(indexDir, 'architecture-summaries.json');
 ...
 */
export function needsSummaryRefresh(
  indexDir: string,
  modules: ModuleInfo[],
...
export function searchSummaries(
  ...
  query: string
): { type: 'project' | 'module'; content: string; module?: ModuleSummary }[] {
  const results: { type: 'project' | 'module'; content: string; module?: ModuleSummary }[] = [];
