...
import {
  estimateContextualCost,
  formatCostEstimate,
  shouldEnableContextual,
  estimateTotalTokens,
...
describe('estimateContextualCost', () => {
  ...
  it('should scale cost with number of chunks', () => {
    const smallChunks: Chunk[] = Array.from({ length: 10 }, (_, i) => ({
      id: `test.ts:${i * 10}-${i * 10 + 10}`,
      content: `function func${i}() {}`,
      filePath: 'src/test.ts',
      lineRange: [i * 10, i * 10 + 10] as [number, number],
      language: 'typescript',
      type: 'code' as const,
    }));
  ...
  it('should benefit from caching across multiple chunks per file', () => {
    ...
    const singlePerFile: Chunk[] = Array.from({ length: 10 }, (_, i) => ({
      id: `file${i}.ts:1-10`,
      content: `function func${i}() {}`,
    ...
    const multiplePerFile: Chunk[] = Array.from({ length: 10 }, (_, i) => ({
    ...
    expect(multipleEstimate.estimatedCacheReadTokens).toBeGreaterThan(
      singleEstimate.estimatedCacheReadTokens
    );
    // And fewer cache writes
    expect(multipleEstimate.estimatedCacheWriteTokens).toBeLessThan(
      singleEstimate.estimatedCacheWriteTokens
    );
  });
});
...
describe('formatCostEstimate', () => {
  it('should format estimate as readable string', () => {
    const estimate: ContextualCostEstimate = {
      totalChunks: 100,
      totalFiles: 10,
    ...
    for (let f = 0; f < 50; f++) {
      for (let c = 0; c < 10; c++) {
        chunks.push({
          ...
          lineRange: [c * 10, c * 10 + 10] as [number, number],
          language: 'typescript',
          type: 'code' as const,
        });
      }
