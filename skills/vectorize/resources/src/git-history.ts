...
const DEFAULT_CONFIG: GitHistoryConfig = {
  maxCommits: 500,
...
export async function getCommitHistory(
  projectRoot: string,
  config: Partial<GitHistoryConfig> = {}
): Promise<CommitInfo[]> {
  ...
  try {
    ...
    const { stdout } = await execAsync(
      `git log -n ${mergedConfig.maxCommits} --format="${format}" --name-only`,
      { 
        cwd: projectRoot,
 ...
 */
function mergeBlameRanges(blameInfos: BlameInfo[]): BlameInfo[] {
  if (blameInfos.length === 0) return [];
  
  const merged: BlameInfo[] = [];
...
export async function getFileHistory(
  ...
  try {
    ...
    const { stdout } = await execAsync(
      `git log -n ${maxCommits} --follow --format="${format}" -- "${filePath}"`,
      { cwd: projectRoot, maxBuffer: 10 * 1024 * 1024 }
    );
...
export async function getNewCommits(
  ...
  try {
    ...
    const { stdout } = await execAsync(
      `git log ${lastIndexedCommit}..HEAD --format="${format}" --name-only`,
 ...
 */
export async function indexCommitHistory(
  projectRoot: string,
  config: Partial<GitHistoryConfig> = {},
  lastIndexedCommit: string | null = null
): Promise<{ records: GitHistoryRecord[]; latestCommit: string | null }> {
 ...
 */
export async function getCommitsForFunction(
  projectRoot: string,
  filePath: string,
  functionName: string,
  ...
  try {
    ...
    if (lineRange) {
      ...
      const { stdout } = await execAsync(
        `git log -L ${start},${end}:"${filePath}" --format="${format}" --no-patch -n 50`,
        { cwd: projectRoot, maxBuffer: 10 * 1024 * 1024 }
      );
